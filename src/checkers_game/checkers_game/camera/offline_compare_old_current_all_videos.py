"""Compare old_better vs current detector across all videos one by one.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compare old_better and current detector on all videos")
...
def main():
    ...
    for idx, video_path in enumerate(videos, start=1):
        ...
        while frame_idx < max_frames:
            ...
            if args.freeze_on_detect and (old_corners is not None or cur_result is not None):
                break

            # Default: keep running until we collect enough detections (up to target) for frequency mode.
            if len(old_scores) >= args.target_detections and len(cur_scores) >= args.target_detections:
                break
        ...
        if old_detected is not None or cur_detected is not None:
            print("  -> Result shown. Press any key for next video (ESC to quit all).")
            key2 = cv2.waitKey(0) & 0xFF
            for name in ("old_better result", "current result", "current warped board", "old mode result", "current mode result"):
                try:
